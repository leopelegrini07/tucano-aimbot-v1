local Repo = "https://raw.githubusercontent.com/zTonho/tucano-aimbot-v1-bridgerwestern/refs/heads/main/"
...
local function fetch(path)
    ...
    if fallbackOk and not isBadResponse(fallbackResult) then
        warn(("[tucano-aimbot-v1] %s was not found in your repo. Loaded fallback from Obsidian official repo."):format(path))
        return fallbackResult
    end

    error(
        ("[tucano-aimbot-v1] Failed to fetch %s.\nPrimary: %s -> %s\nFallback: %s -> %s"):format(
            path,
            url,
...
local function run(path)
    ...
    if not chunk then
        error(("[tucano-aimbot-v1] Failed to compile %s: %s"):format(path, tostring(compileError)), 2)
    end

    local ok, result = pcall(chunk)
    if not ok then
        error(("[tucano-aimbot-v1] Failed to run %s: %s"):format(path, tostring(result)), 2)
    end
...
if env.tucano-aimbot-v1 and type(env.tucano-aimbot-v1.Cleanup) == "function" then
    pcall(env.tucano-aimbot-v1.Cleanup)
end

if env.tucano-aimbot-v1 and env.tucano-aimbot-v1.Library and type(env.tucano-aimbot-v1.Library.Unload) == "function" then
    pcall(function()
        env.tucano-aimbot-v1.Library:Unload()
    end)
end
...
Loading:SetMessage("Loading tucano-aimbot-v1")
...
if not ThemeManagerOk then
    warn("[tucano-aimbot-v1] ThemeManager failed to load: " .. tostring(ThemeManager))
    ThemeManager = nil
end
...
if not SaveManagerOk then
    warn("[tucano-aimbot-v1] SaveManager failed to load: " .. tostring(SaveManager))
    SaveManager = nil
end
...
if not MovementOk then
    warn("[tucano-aimbot-v1] Movement setup failed: " .. tostring(MovementError))
    Library:Notify({
        Title = "tucano-aimbot-v1",
...
if ThemeManager then
    ...
    ThemeManager:SetFolder("tucano-aimbot-v1")
...
if SaveManager then
    ...
    SaveManager:SetFolder("tucano-aimbot-v1")
